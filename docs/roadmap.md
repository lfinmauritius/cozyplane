...
- [ ] **Multi-attach — several VPCs on one pod** (`design.md` §9 phase 6; **design landed, implementation open** — [multi-attach.md](multi-attach.md)). A pod takes one VPC today, which is enough for a workload that *lives* in a network and useless for one whose job *is* the boundary between two: a tenant firewall, a router, an NFV appliance. Declared by a JSON-list annotation, not the `NetworkAttachment` CRD (never built, and deliberately dropped — a pod's network layout is a property of that pod, and a namespaced object holding a copy is the stale-copy problem removed twice already). Attachment **order** is contract: entry 0 carries the fabric bridge, the default route and `status.podIP`; the rest get an on-link route to their own CIDR and nothing more, because N default routes pick an egress interface nondeterministically. Carries **static addressing** (`ip`/`mac` per entry) — the piece that could not be expressed at all, since a Port's name *is* its address claim and was only ever allocated by walking the CIDR. **The extra interfaces are the easy half:** `from_pod`'s source RPF drops a forwarded packet at its origin veth, and exempting an interface from it is the right to impersonate any member of that VPC — so it is `VPCBinding.allowForwarding`, authored by whoever holds `export` on the VPC, not requested by the workload. Related to the authorized-forwarder role in #6. The extra **interfaces** cost the datapath nothing (`ports`/`locals` are already keyed per interface), but the **grant does**: an earlier draft claimed `PORT_F_GATEWAY` could be reused and that was a security hole, not a shortcut — the gateway flag also stamps `GW_MARK`, on which `to_pod` SKIPS the SecurityGroup gate, so the grant silently disabled policy for the one workload that most needs it (measured live). Tenant forwarding is therefore its own `PORT_F_FORWARD`/`FWD_MARK`/`TUN_F_FORWARD`: same RPF exemption, marked only when the source is genuinely foreign, clears isolation, and is still judged by the destination's SecurityGroups — as a north-south source, via the existing `ns_sg_admit`, since this VPC holds no identity for an address it does not own. Known limits recorded in the doc: DNS steering and kubelet probes ride entry 0 only, and NS_GW metering will attribute a forwarding leg's east-west to the north-south door until the counter keys on the Port's role rather than the datapath flag
- [ ] **Multi-NIC KubeVirt VMs — the Multus adapter** (`design.md` §9; **design landed, implementation open** — [kubevirt-multi-nic.md](kubevirt-multi-nic.md)). Multi-attach above serves *pods*; a VM is a guest inside the `virt-launcher` pod and sees only the NICs KubeVirt put in its domain. KubeVirt declares only what `spec.networks` names, and that field admits exactly `pod: {}` and `multus: {}` — so an interface cozyplane adds to the launcher without a matching entry is configured, addressed and never bridged into the guest. Multus is not one way to give a VM a second NIC, it is the only way, which is why §9's "Multus goes away" is a claim about pods and stays one. Two pieces: a **delegate mode** in the plugin, discriminated by a `vpc` key in the CNI config, that builds exactly one never-primary attachment on `args.IfName` and whose DEL removes only its own interface (today's DEL enumerates every index — a delegated DEL would take `eth0` with it); and a shim **`NetworkAttachmentDefinition` generated per `VPCBinding`**, so it lands where attachment is already authorized and adds no authorization surface, conditional on the cluster serving `k8s.cni.cncf.io` via the same `RESTMapping` gate the persistent-Port controller uses for the VMI. Host veths take a disjoint name space (letter suffix, not digit) so the two DEL paths cannot reconstruct each other's names. Static addressing stays in the `sdn.cozystack.io/networks` annotation, keyed by the Multus interface name, with `net[0-9]+` reserved. Known limit carried forward: **secondary NICs are not live-migratable** — [live-migration.md](live-migration.md)'s primary-network scope is unchanged and KubeVirt restricts this itself
