...
- [x] **Site-to-site VPN core** — scoped authorized-forwarder, per-VPC route table,
  managed WireGuard and route-based IPsec ([vpn.md](vpn.md), [#6](../../issues/6)).
  Warm-standby HA and resource guardrails are built; live-migration/active-active
  HA and roadwarrior access remain follow-ups. Stock Cozystack/Talos supports
  WireGuard; IPsec fails closed until its kernel enables `CONFIG_XFRM_INTERFACE`.
